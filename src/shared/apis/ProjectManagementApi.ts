...
import { IProjectLogs } from "../models/ProjectLogs.model";
...
export default class ProjectManagementApi {
    ...
    async updateTaskGantt(projectId: string, taskId: string, value: { [x: string]: any; }) {
    ...
    async deleteFile(projectId: string, task: IProjectTask, index: number) {
        const path = this.taskPath(projectId);
        if (!path || !task.files) return;

        const { link } = task.files[index];

        try {
            await deleteObject(ref(storage, link));
            task.files.splice(index, 1);
            await updateDoc(doc(collection(db, path), task.id), { files: task.files });
            this.store.projectTask.load([task]);
        } catch (error) { }
    }
    ...
    //   In this example, the function checks if the files property of the task object is defined and not empty. If there are files associated with the task, it loops through all the files in the array and deletes them one by one using the deleteObject function. Then, it proceeds to delete the task document from the database and remove it from the store.
    ...
    // async addRisk(projectId: string, value: any) {

    //     const path = this.riskPath(projectId);
    //     if (!path) return;

    //     try {
    //         const taskRef = collection(db, path);
    ...
    // async updateRisk(projectId: string, riskId: string, value: string | any) {

    //     const path = this.riskPath(projectId);
    //     if (!path) return;

    //     const ref = doc(collection(db, path, riskId));
    //     await updateDoc(ref, value);
    //     this.getRisks(projectId);
    // }


    //"smulenamaswe@ecb.org.na"
