...
export default class StrategicThemeApi {
  ...
  private getPath(scorecard?: string) {
    ...
    return apiPathScorecardLevel(this.store.scorecard.activeId, "strategicThemes");
  }
  ...
  async getAll(scorecard: string) {
    ...
    this.store.strategicTheme.removeAll()
    // create the query
  ...
  async create(item: IStrategicTheme) {
    ...
    item.id = itemRef.id;
    // added
    item.scorecard = this.store.scorecard.activeId!;
    // create in db
    try {
  ...
  // update item
  async update(item: IStrategicTheme, fieldsUpdated?: (keyof IStrategicTheme)[]) {
    const path = this.getPath();
    if (!path) return;
    ...
    try {
      ...
      if (!fieldsUpdated) {
        ...
        const currItem = this.store.strategicTheme.getItemById(item.id);
        const newItem = currItem ? { ...currItem.asJson, ...strategicTheme } : item;
