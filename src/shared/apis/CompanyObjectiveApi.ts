...
export default class CompanyObjectiveApi {
  ...
  //   return apiPathScorecardLevel(this.store.scorecard.activeId, "companyObjectives");
  // }

  private currentPath() {
    if (!this.store.scorecard.currentId) return;
    return apiPathScorecardLevel(this.store.scorecard.currentId, "companyObjectives");
  }
  ...
  // duplicate objective
  async duplicate(item: IObjectiveCompany) {
