...
export default class CompanyMeasureApi {
  ...
  //   return apiPathScorecardLevel(this.store.scorecard.activeId, "companyMeasures");
  // }


  private currentPath() {
    if (!this.store.scorecard.currentId) return;
    return apiPathScorecardLevel(this.store.scorecard.currentId, "companyMeasures");
  }
  ...
  async duplicate(item: IMeasureCompany) {
  ...
  async update(
    ...
    try {
      ...
      if (!fieldsUpdated) {
        ...
        const currItem = this.store.companyMeasure.getItemById(item.id);
