...
export default class CompanyScorecardReviewDraftApi {
  constructor(private api: AppApi, private store: AppStore) { }

  private getPath() {
    if (!this.store.scorecard.activeId) return null;
    return apiPathScorecardLevel(this.store.scorecard.activeId, "scorecardDraftReviews");
  }
  ...
  async getByUid(id: string) {
    ...
    const unsubscribe = onSnapshot(doc(db, path, id), (doc) => {
      ...
      this.store.companyScorecardReview.draft.load([item]);
    });
    ...
    return await new Promise<Unsubscribe>((resolve, reject) => {
      ...
      const unsubscribe = onSnapshot(
        ...
        (querySnapshot) => {
          ...
          this.store.companyScorecardReview.draft.load(items);
          resolve(unsubscribe);
        },
        // onError
        (error) => {
  ...
  async delete(item: IScorecardReview) {
    ...
    try {
      ...
      // remove from store
      this.store.companyScorecardReview.draft.remove(item.uid); // Remove from memory
    } catch (error) {
      // console.log(error);
    }
  }
