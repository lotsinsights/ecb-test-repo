...
export default class IndividualScorecardApi {
  ...
  private getPath() {
    ...
    return apiPathScorecardLevel(this.store.scorecard.activeId, "scorecardMetadata");
  }
  ...
  async getByUid(id: string, notFound?: () => void) {
    ...
    const unsubscribe = onSnapshot(doc(db, path, id), (doc) => {
      ...
      this.store.individualScorecard.load([item]);
    });
  ...
  async getAll() {
    ...
    return await new Promise<Unsubscribe>((resolve, reject) => {
      ...
      const unsubscribe = onSnapshot($query, (querySnapshot) => {
        ...
        this.store.individualScorecard.load(items);
        resolve(unsubscribe);
      },
        // onError
        (error) => {
  ...
  async delete(item: IScorecardMetadata) {
    ...
    try {
      ...
      // remove from store
      this.store.individualScorecard.remove(item.uid); // Remove from memory
    } catch (error) {
      // console.log(error);
    }
  }
