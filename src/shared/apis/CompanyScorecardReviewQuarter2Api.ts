...
export default class CompanyScorecardReviewQuarter2Api {
  ...
  async getByUid(id: string) {
    ...
    const unsubscribe = onSnapshot(doc(db, path, id), (doc) => {
      ...
      this.store.companyScorecardReview.quarter2.load([item]);
    });
    ...
    return await new Promise<Unsubscribe>((resolve, reject) => {
      ...
      const unsubscribe = onSnapshot(
        ...
        (querySnapshot) => {
          ...
          this.store.companyScorecardReview.quarter2.load(items);
          resolve(unsubscribe);
        },
        // onError
        (error) => {
  ...
  async delete(item: IScorecardReview) {
    ...
    try {
      ...
      // remove from store
      this.store.companyScorecardReview.quarter2.remove(item.uid); // Remove from memory
    } catch (error) {
      // console.log(error);
    }
  }
