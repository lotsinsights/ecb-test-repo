import { S1Departments } from "./Departments";
import { observer } from "mobx-react-lite";
import { useAppContext } from "../../../shared/functions/Context";
import { IDepartmentPeformanceData } from "../../../shared/models/Report";

const DepartmentS1Performance = observer(() => {
  ...
  const departmentPerformance = (): IDepartmentPeformanceData[] => {
    const data = store.department.all.map((department) => {
      ...
      const avg = performanceData.reduce((acc, dep) => acc + (dep.asJson.rating), 0) / performanceData.length;
      const avgRounded = Math.round(avg * 10) / 10 || 0;
      // weight
      const weight = performanceData.reduce((acc, dep) => acc + dep.asJson.weight, 0) || 0;
      // min
      const min = performanceData.reduce((acc, dep) => (acc < dep.asJson.rating ? acc : dep.asJson.rating || 0), 1);
      // max
      const max = performanceData.reduce((acc, dep) => (acc > dep.asJson.rating ? acc : dep.asJson.rating || 0), 1);

      //median
      const performanceDataLength = performanceData.length;
      const median = performanceDataLength !== 0 ? performanceDataLength % 2 === 0 ? ((performanceData[performanceDataLength / 2 - 1].asJson.rating ||
        0) + performanceData[performanceDataLength / 2].asJson.rating || 0) / 2 : performanceData[(performanceDataLength - 1) / 2].asJson.rating || 0 : 1;
  ...
  return (
    <div className="department-tab-content">
      <div
        ...
        <div>
          <S1Departments data={departmentPerformance()} />
        </div>
      </div>
    </div>
  );
...
export default DepartmentS1Performance;
