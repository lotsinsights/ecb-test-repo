...
const ReviewPeople = observer((props: IReviewPeopleProps) => {
  ...
  const onExportExcel = async () => {
    await exportPerformanceOverview(filterScorecards());
  };
  ...
  const filterScorecards = useCallback(() => {
    if (selectedDepartment === "all") return scorecards;
    else return scorecards.filter((s) => s.department === selectedDepartment);
  }, [scorecards, selectedDepartment]);
    ...
    <ErrorBoundary>
      <div className="review-staff">
        ...
        <table className="people-table uk-table uk-table-small uk-table-middle uk-table-hover uk-table-divider">
          ...
          <tbody>
            {filterScorecards().map((scorecard, index) => (
              <tr
                className="row"
                key={scorecard.uid}
