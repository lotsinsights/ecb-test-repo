...
const FolderItem = ({ index, folder, path }: IProps) => {
  ...
  const [copyLinkBtnText, setCopyLinkBtnText] = useState<"Copy Link" | "Copied">("Copy Link");
  ...
  useEffect(() => {
    ...
    if (me.role === USER_ROLES.HR_USER || me.role === USER_ROLES.ADMIN_USER) {
      setisLocked(false); // if user is  user, then unlock
    } else {
      ...
      if (folder.department === me.department) {
        if (me.role === USER_ROLES.GENERAL_MANAGER || me.role === USER_ROLES.MANAGER_USER) {
          setisLocked(false);
