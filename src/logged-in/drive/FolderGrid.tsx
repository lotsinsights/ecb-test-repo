...
const FolderGrid = observer((props: IProps) => {
  ...
  const folders = store.folder.all.filter((folder) => folder.asJson.parentId === parent);
  ...
  //   if (role === USER_ROLES.HR_USER || role === USER_ROLES.ADMIN_USER) {
  //     return sorted;

  //   } else if (role === USER_ROLES.HR_USER || role === USER_ROLES.ADMIN_USER) {
  //     let $folders = sorted.filter((f) => {
  //       return f.asJson.department === department;
  //     });

  //     if (parent === "root") return $folders;
  ...
  //       $folders = $folders.filter((f) => subs.includes(f.asJson.id));
  //       return $folders;
  //     }
  ...
  const filterAccess = useMemo(() => {
    ...
    if (role === USER_ROLES.HR_USER || role === USER_ROLES.ADMIN_USER) {
      return sorted;
    } else if (role === USER_ROLES.MANAGER_USER || role === USER_ROLES.GENERAL_MANAGER || role === USER_ROLES.EMPLOYEE_USER) {
      let $folders = sorted.filter((f) => {
        return f.asJson.department === department;
      });
      ...
      return store.folder.all.filter((f) => {
        return f.asJson.department === department;
      });
    } else {
      return [];
  ...
  useEffect(() => {
    const setFolderPath = () => {
      ...
      const currentFolder = store.folder.getItemById(parent);
  ...
  useEffect(() => {
    ...
    const loadCurrentFolder = async () => {
      // if (parent === "root") return; hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh TO DO
