...
const CompanyExcelDefinition = (
  ...
  const tableRows: ICompanyAgreementTableRowItem[] = measures.map((m) => {
    ...
    const row: ICompanyAgreementTableRowItem = {
      ...
      rating: m.rating || 0,
      q2supervisorRating: m.q2supervisorRating || 0,
      q2FinalRating: m.q2FinalRating || 0,
      q4rating: m.q4rating || 0,
      q4supervisorRating: m.q4supervisorRating || 0,
      q4FinalRating: m.q4FinalRating || 0
    };
...
export const exportCompanyScorecardExcel = async (
  ...
  const dataRows = rows.map((row, _, data) => {
    ...
    if (objective !== row.description) {
      ...
      objectiveRowSpan = data.filter((r) => r.description === objective).length;
    }
    ...
    return [
      ...
      formatColumn({ value: row.rating, type: "Number" }), // E
      formatColumn({ value: row.q2supervisorRating, type: "Number" }), // S
      formatColumn({ value: row.q2FinalRating, type: "Number" }), // F
      formatColumn({ value: row.q4rating, type: "Number" }), // E
      formatColumn({ value: row.q4supervisorRating, type: "Number" }), // S
      formatColumn({ value: row.q4FinalRating, type: "Number" }), // F
    ];
  });
