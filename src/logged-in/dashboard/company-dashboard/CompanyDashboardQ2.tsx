...
import { totalQ2CompanyObjectiveRating, totalQ2MeasureCompanyRating,totalQ4MeasureCompanyRating } from "../../shared/functions/Scorecard";
    ...
    return (
        <li className="red-measure">
            <div className="uk-flex uk-flex-middle">
                <Rating rate={(measure.q2FinalRating || 0)} isUpdated={measure.isUpdated} />
...
const ObjectiveList = observer((props: ImProps) => {
    ...
    const calculateRating = () => {
        ...
        const rating = totalQ2CompanyObjectiveRating(measures);
        return {
            rate: rating,
...
const MetricsAnalytics = observer(() => {
    ...
    const redMeasures = measures.filter((measure) => (measure.asJson.q2FinalRating || 0) <= 2);
...
const MeasuresAnalytics = observer(() => {
    ...
    const redMeasures = measures.filter((measure) => (measure.asJson.q2FinalRating || 0) < 2);
    const greenMeasures = measures.filter((measure) => (measure.asJson.q2FinalRating || 0) >= 3);
    const amberMeasures = measures.filter((measure) => (measure.asJson.q2FinalRating || 0) < 3 && (measure.asJson.q2FinalRating || 0) >= 2);
...
export const CompanyDashboardQ2 = observer(() => {
    return (
        <ErrorBoundary>
            <div>
                <MetricsAnalytics />
