...
import { totalQ2MeasureRating, totalQ4IndividualObjectiveRating, totalQ4MeasureRating } from "../../shared/functions/Scorecard";
    ...
    return (
        <li className="red-measure">
            <div className="uk-flex uk-flex-middle">
                <Rating rate={(measure.finalRating2 || 0)} isUpdated={measure.isUpdated} />
...
const ObjectiveItem = observer((props: ImProps) => {
    ...
    const calculateRating = () => {
        ...
        const _measures = measures.filter((measure) => measure.isUpdated).length > 0;
...
const MeasuresAnalytics = observer(() => {
    ...
    const redMeasures = measures.filter((measure) => (measure.asJson.finalRating2 || 0) < 2);
    const greenMeasures = measures.filter((measure) => (measure.asJson.finalRating2 || 0) >= 3);
    const amberMeasures = measures.filter((measure) => (measure.asJson.finalRating2 || 0) < 3 && (measure.asJson.finalRating2 || 0) >= 2);
...
export const IndividualDashboardQ4 = observer(() => {
    return (
        <ErrorBoundary>
            <MetricsAnalytics />
            <ObjectivesAnalytics />
