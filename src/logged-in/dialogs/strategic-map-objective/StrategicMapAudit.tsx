...
import { totalQ4IndividualObjectiveRating } from "../../shared/functions/Scorecard";
...
const StrategicMapAudit = observer((props: IProps) => {
  ...
  const data = months.map((month) => {
    ...
    const measuresAudit = store.measureAudit.allMe.filter((m) => mapMonthToAbbreviation(m.asJson.month) === month).map((m) => m.asJson);

    const rating = totalQ4IndividualObjectiveRating(measuresAudit) || null;
    return rating;
  });
  ...
  const loadMeasuresAuditData = useCallback(async () => {
    try {
      await api.measureAudit.getByObjective(objective.id);
    } catch (error) {
    }
  }, [api.measureAudit, objective.id]);
