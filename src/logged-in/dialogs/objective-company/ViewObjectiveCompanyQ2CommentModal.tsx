...
const ViewObjectiveCompanyQ2CommentModal = observer(() => {
  const { store } = useAppContext();
  const [objective, setObjective] = useState<IObjectiveCompany>({ ...defaultObjectiveCompany })

  const onCancel = () => {
    store.companyObjective.clearSelected()
    hideModalFromId(MODAL_NAMES.EXECUTION.VIEW_OBJECTIVE_COMPANY_Q2_COMMENT_MODAL);
  };
...
export default ViewObjectiveCompanyQ2CommentModal;
