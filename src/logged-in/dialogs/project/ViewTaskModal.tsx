...
import { IProjectTask, IProjectTaskStatus, defaultTask } from "../../../shared/models/ProjectTasks.model";
...
const ViewTaskModal = observer(() => {
  ...
  const onDeleteFile = async (index: number) => {
    ...
    setLoading(true);
    try {
      await api.projectManagement.deleteFile(task.projectId, task, index);
    } catch (error) {
      setLoading(false);
    }
    setLoading(false);
  ...
  const onRemoveMember = async (index: number) => {
    ...
    const newUsersId = task.usersId.filter((_, i) => i !== index);
    const _task: IProjectTask = {
      ...task,
      usersId: newUsersId
    }
            ...
            <ul className="uk-list">
              {task.files?.map((file, index) => (
                <li key={index}>
                  ...
                  {task.usersId[0] === me.uid && (
                    <button className="delete" data-uk-tooltip="Remove"
