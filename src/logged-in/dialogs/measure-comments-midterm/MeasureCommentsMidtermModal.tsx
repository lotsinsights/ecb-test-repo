...
const MeasureCommentsMidtermModal = observer(() => {
  ...
  const update = async (measure: IMeasure) => {
    try {
      await api.measure.update(measure, ["midtermComments"]);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const onCancel = () => {
    ...
    hideModalFromId(MODAL_NAMES.EXECUTION.MEASURE_COMMENTS_MIDTERM_MODAL);
  };
  ...
  useEffect(() => {
    ...
    if (maxCharCount < measure.midtermComments.length) {
      // clip the midtermComments to maxCharCount
      setMeasure({
        ...measure,
  ...
  return (
    <div className="measure-midtermComments-modal uk-modal-dialog uk-modal-body uk-margin-auto-vertical">
      <button
        ...
        >
          <div className="uk-width-1-1">
            <textarea
              ...
              rows={10}
              placeholder="KPI Midterm Comments"
...
export default MeasureCommentsMidtermModal;
