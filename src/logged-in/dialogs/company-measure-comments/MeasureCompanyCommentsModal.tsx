...
const MeasureCompanyCommentsModal = observer(() => {
  ...
  const update = async (companyMeasure: IMeasureCompany) => {
    try {
      await api.companyMeasure.update(companyMeasure, ["comments"]);
    } catch (error) {
      ui.snackbar.load({
        id: Date.now(),
  ...
  const onCancel = () => {
    ...
    hideModalFromId(MODAL_NAMES.EXECUTION.MEASURE_COMMENTS_COMPANY_MODAL);
  };
...
export default MeasureCompanyCommentsModal;
