...
const PerformanceMidtermApprovalModal = observer(() => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const { MY_SUBJECT, MY_BODY } = MAIL_MIDTERM_APPROVED_ME(user.displayName, me.displayName);

    setLoading(true);
    const $agreement: IScorecardMetadata = { ...agreement };
    $agreement.quarter2Review.comments = appraisal;
  ...
  const onCancel = () => {
    ...
    hideModalFromId(MODAL_NAMES.EXECUTION.PERFORMANCE_MIDTERM_APPROVAL_MODAL);
  };
  ...
  return (
    <div className="scorecard-approval-modal uk-modal-dialog uk-modal-body uk-margin-auto-vertical">
      <button
        ...
        data-uk-close
      ></button>
      <h3 className="uk-modal-title">Approve Midterm</h3>
...
export default PerformanceMidtermApprovalModal;
