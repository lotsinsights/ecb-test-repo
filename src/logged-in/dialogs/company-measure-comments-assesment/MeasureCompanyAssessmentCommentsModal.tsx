...
const MeasureCompanyAssessmentCommentsModal = observer(() => {
  ...
  const update = async (companyMeasure: IMeasureCompany) => {
    try {
      await api.companyMeasure.update(companyMeasure, ["assessmentComments"]);
    } catch (error) {
      ui.snackbar.load({
        id: Date.now(),
  ...
  const onCancel = () => {
    ...
    hideModalFromId(MODAL_NAMES.EXECUTION.MEASURE_COMMENTS_COMPANY_ASSESSMENT_MODAL);
  };
...
export default MeasureCompanyAssessmentCommentsModal;
