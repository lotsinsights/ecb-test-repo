...
import { defaultRisk, IProjectRisk, IRiskStatus } from "../../shared/models/ProjectRisks.model";
import AddUserModal from "../dialogs/project/AddUsersModal";
import FilesModal from "../dialogs/project/FilesModal";
import NewRiskModal from "../dialogs/project/NewRiskModal";
import ViewTaskModal from "../dialogs/project/ViewTaskModal";
...
import {
    ...
    getInitials, getProgressClass, getProgressColors, getRemainingTime, groupRisks, groupTasks,
} from "./utils/common";
import NewMilestoneModal from "../dialogs/project/NewMilestoneModal";
import ProjectMilestoneTasks from "./components/projectMilestoneTasks";
import ViewRiskModal from "../dialogs/project/ViewRiskModal";
import { MAIL_EMAIL, MAIL_PROJECT_REMOVED } from "../../shared/functions/mailMessages";
...
import { IProjectTask } from "../../shared/models/ProjectTasks.model";
import { ProjectGridRiskItem } from "./components/items/ProjectRiskGridItem";
import "react-circular-progressbar/dist/styles.css";
import { IProjectLogs } from "../../shared/models/ProjectLogs.model";
...
const Project = observer(() => {
    ...
    const onRemoveMember = async (userId: string, index: number) => {
        ...
        const { MY_SUBJECT, MY_BODY } = MAIL_PROJECT_REMOVED(me?.displayName!, project.projectName);
        ...
        try {
            ...
            await api.mail.sendMail([user!.asJson.email!], MAIL_EMAIL, MY_SUBJECT, MY_BODY);
        ...
        <ErrorBoundary>
                            ...
                            <div className="milestone">
                                ...
                                <ul uk-accordion="multiple: true">
                                    {milestones.map((milestone) => (
                                        <li className="uk-close milestone-list" key={milestone.id} >
                                            <div className={`m-status ${milestone.status}`}>
                                                {milestone.status}
                                            </div>
                                            <a className="uk-accordion-title">
                                                <div className="milestone-title">
                                                    <span className="m-name">{milestone.taskName}</span>
                                                    <span className={`uk-badge badge ${milestone.status}`}>
                                                        {milestone.status}
                                                    </span>
                                                </div>
                                            </a>
            ...
            <ErrorBoundary>
                <Sidebar visible={visible} position="right" onHide={() => setVisible(false)} >
                    <h3>Logs</h3>
                    {project.manager === me?.uid && logs.length !== 0 &&
