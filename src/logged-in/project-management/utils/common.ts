...
import { IProjectRisk } from "../../../shared/models/ProjectRisks.model";
...
//         rating: Number(measure.finalRating2 || measure.autoRating2),
//     }));
...
// export function projectsMilestonesTotal(projects: IProject[], tasks: IProjectTask[]) {

//     const data = tasks.filter((task) => task.type === "milestone");

//     const projectBudget = projects.map((project) => project.awardedAmount)

//     const totalBudget = projectBudget.reduce((acc, curr) => {
//         return acc + curr || 0
//     }, 0) || 0;
...
//     return { amountleft, completedTaskCost, percentage };
// }
// style={{ backgroundColor: getMilestoneColors(item.status), textTransform: "capitalize" }}>
...
export function getTaskProgress(tasks: IProjectTask[] | any) {
    ...
    // return ([
    //     { category: "Todo", value: todo },
    //     { category: "In progress", value: inProgress },
    //     { category: "In Review", value: inReview },
    //     { category: "Done", value: done }
    // ]);
